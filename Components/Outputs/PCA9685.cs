...
namespace Scarlet.Components.Outputs
{
    ...
    public class PCA9685
    {
        ...
        private const byte AllLEDRegisterOffset = 0xFA;
        ...
        public PCA9685(II2CBus Bus, byte Address, int ExtOscFreq = -1)
        {
            ...
            if (ExtOscFreq < -1 || ExtOscFreq > 50000000) { throw new ArgumentException("Invalid oscillator frequency supplied. Must be bwetween 0 and 50 MHz, or -1 for internal oscillator."); }
        ...
        {
            byte[] OutputData = this.Bus.ReadRegister(this.PartAddress, FirstLEDRegisterOffset, 64);
        ...
        /// <param name="Frequency"> This is used in software to determine some timing values. Provide the actual frequence of the attached oscillator. </param>
        public void UseExternalOscillator(int Frequency)
        {
            ...
            byte ModeSettingPreExt = this.Bus.ReadRegister(this.PartAddress, FirstModeRegisterOffset, 1)[0];
            byte ModeSettingNewExt = (byte)((ModeSettingPreExt & 0b0111_1111) | 0b0101_0000); // Set bits 6 and 4 (EXTCLK & SLEEP), but not bit 7 (SLEEP).
            this.Bus.WriteRegister(this.PartAddress, FirstModeRegisterOffset, new byte[] { ModeSettingNewExt });
        ...
        internal void SetChannelData(byte Channel, byte[] Data)
        {
            ...
            byte Register;
            if (Channel == 255) { Register = AllLEDRegisterOffset; }
            else { Register = (byte)((Channel * 4) + FirstLEDRegisterOffset); } // Single LED channel
